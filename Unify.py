...
if __name__ == "__main__":
    print("Process unify start time: ", datetime.now().strftime("%H:%M:%S"))
    ...
    result.points.extend(p_all)
    result.colors.extend(c_all)

    # 合成された点群をXYZフォーマットでファイルに出力
    o3d.io.write_point_cloud(output_data, result)
    print("Process unify end time: ", datetime.now().strftime("%H:%M:%S"))
